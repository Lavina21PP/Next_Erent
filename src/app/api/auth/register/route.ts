  ...
  try {
    ...
    if (!email_phone) {
      return NextResponse.json(
        { success: false, message: "ກະລຸນາປ້ອນອີເມວ ຫຼື ເບີໂທລະສັບ" },
        { status: 400 }
      );
    }
    ...
    if (user.length > 0) {
      return NextResponse.json(
        { success: false, message: "ມີຜູ້ໃຊ້ນີ້ແລ້ວ" },
        { status: 409 }
      );
    }
    ...
    try {
      ...
      return NextResponse.json(
        { success: false, message: "ບໍ່ສາມາດສົ່ງອີເມວໄດ້" },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json(
      { success: false, message: "ເກີດຂໍ້ຜິດພາດທີ່ server" },
      { status: 500 }
    );
  }
}
