...
function PopConfirm({
  ...
  setOpenPopConfirm: (value: boolean) => void;
  text: string;
}) {

  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Enter" && openPopConfirm) {
        onConfirm();
        setOpenPopConfirm(false); // ปิด modal หลัง confirm
      }
      if (e.key === "Escape" && openPopConfirm) {
        setOpenPopConfirm(false); // กด Escape ปิด modal
      }
    };
  ...
  }, [openPopConfirm, onConfirm, setOpenPopConfirm]);
  return (
    <div>
      {openPopConfirm && (
        <div
